...
if (all(metadata[,1] == colnames(abundance_matrix))) {
  ...
  metadata <- metadata[order(as.character(metadata$sample_name)),]
...
write.table(round(abundance_matrix_rpm, digits=4), file=paste(prefix, "agg_matrix.txt", sep="_"), quote=F, sep="\t")
